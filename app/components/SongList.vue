...
const toggleLike = async (song: Song) => {
  const result: any = await $api(`/songs/${song.id}/like`, {
    method: 'POST',
    headers: {
  ...
  song.liked = result.liked
  await refreshNuxtData('songs')
}
</script>
...
<template>
  <div class="flex flex-col">
    <div v-for="(song, index) in songs" :key="song.id"
      ...
      <img :src="song.coverUrl" class="w-10 h-10 rounded object-cover shrink-0 cursor-pointer"
      ...
      <button @click="toggleLike(song)">
          <UIcon :name="'i-lucide-heart'" :class="song.liked ? 'text-red-500 fill-red-500' : 'text-gray-400'" />

          {{ song.likes }}
      </button>
      <button v-if="auth.isLoggedIn" class="text-gray-400 hover:text-white opacity-0 group-hover:opacity-100 shrink-0"
        @click="toggleMenu(song.id)">
        <UIcon name="i-lucide-plus" class="w-5 h-5" />
      </button>

      <div v-if="showMenu === song.id" class="absolute right-0 top-10 bg-gray-800 rounded shadow-lg p-2 z-10 w-48">
        <p v-if="!playlists.length" class="text-xs text-gray-400 p-2">Chưa có playlist nào</p>
        <button v-for="p in playlists" :key="p.id"
